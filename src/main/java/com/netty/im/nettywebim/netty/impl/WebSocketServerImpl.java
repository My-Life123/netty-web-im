package com.netty.im.nettywebim.netty.impl;

import com.netty.im.nettywebim.common.GlobleUtils;
import com.netty.im.nettywebim.netty.NettyHttpService;
import com.netty.im.nettywebim.netty.WebSocketServerHandler;
 ...
 */
@Service
public class WebSocketServerImpl implements WebSocketService, NettyHttpService {
    ...
    private static String WEBSOCKET_URI_ROOT;
    ...
    public void start(String host,int port){
        ...
        sb.childHandler(new ChannelInitializer<Channel>() {
            ...
            protected void initChannel(Channel ch) throws Exception {
                ...
                group.add(ch);
                GlobleUtils.socketChannelMap.put(ch.id().asShortText(),ch);
                ch.closeFuture().addListener(new ChannelFutureListener() {
                    ...
                    public void operationComplete(ChannelFuture future) throws Exception {
                        ...
                        GlobleUtils.socketChannelMap.remove(future.channel().id().asShortText());
                        group.remove(ch);
                    }
                });
...
//
    ...
    public boolean sendMessage(String channelId, String message) {
        TextWebSocketFrame rsp = new TextWebSocketFrame(message);
        if(GlobleUtils.socketChannelMap.get(channelId)!=null) {
            GlobleUtils.socketChannelMap.get(channelId).writeAndFlush(rsp); //发送给指定客户端
            return true;
        }else{
            return false;
        }
